...
Use Java or C# to design and implement a desktop application for Chatting. The application should have two types of users (a regular user and an administrator user) which have to provide a username and a password in order to use the application.

The regular user can perform the following operations:
* Create new chat room
* View existing chat rooms and write messages in there

The administrator can perform the following operations:
* CRUD on chat rooms
* CRUD on regular users' information.
* Generate two types of report files, one in pdf format and one in txt or html format, with the rooms and the conversations. The reports **need** to be saved in a **user-selected location** (not predefined by the application), similar on how one would save a file from Notepad.
...
New features:
* Users can star a channel so it appears first in the list of channels he joined
* An user gets a live notification when a message is written in a chat, even if he's not there
* An user gets a notification when a person joins/leaves the *current* chat room he's in
...
* All the inputs of the application will be validated against invalid data before submitting the data and saving it.
* Unit test the chat-room-related controller(s)/view model(s) using a testing framework (e.g., JUnit, NUnit). Use a Mocking framework (e.g., Mockito, Moq) to mock dependencies on repositories/views/server connection. The tests **should not** need direct access to the client or the server.
...
Final features:
* Users can send direct private message to a specific user, without the need for a new chat room. (Technically they would function similarly)
* An user can block another user from receiving private messages.
* Chat room owners (creators) can specify one of the following permissions: only allow the list of users or deny this list of users (the lists are configurable)
